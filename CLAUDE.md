...
- **Every Gemini call goes through `callGemini` in `backend/geminiService.ts`** — it counts calls, caps them per day and logs usage. **Two caps, because two different risks**: `GEMINI_MAX_CALLS_PER_DAY` (default 200) is a runaway detector — tokens on Gemini 3 Flash are cheap; `GEMINI_MAX_SEARCH_CALLS_PER_DAY` (default 25) is the one that guards money, because Google Search grounding costs $14/1000 requests past the free 5000/month and that is what produced the September bill. The counter lives in its own `gemini-usage.json` in `DATA_DIR` (atomic write, survives restarts, day counted in Europe/Warsaw); an unreadable counter file **blocks calls** rather than silently resetting. Reset manually via `POST /api/gemini-usage/reset`. **The binding limit is `GEMINI_MAX_COST_PLN_PER_MONTH` (default 5 zł)** — real cost in PLN, accumulated per billing month from each response's `usageMetadata` using measured unit prices in `constants.ts` (11,14 zł/M output tokens, 1,86 zł/M input — derived from the September 2026 invoice, not the price list). Search stays free by keeping grounded calls inside Google's 5000/month free pool (`GEMINI_MAX_SEARCH_CALLS_PER_MONTH`, default 900) and logs model, search usage, `finishReason` and token spend. Never call `ai.models.generateContent` directly again: on 10–11 Sep 2026 the app made ~4700 calls/day instead of ~110 and ran up a 564 zł bill with **no trace in the logs**. Full write-up and open questions: `INCYDENT-2026-09-koszty-gemini.md`.
