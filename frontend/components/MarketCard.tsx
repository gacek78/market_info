  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-3 pr-6">
        <span className="font-bold text-base text-blue-400 truncate">{etf.ticker}</span>
        <span className="text-[8px] px-1.5 py-0.5 bg-slate-900/50 rounded text-slate-500 uppercase font-black border border-slate-700/50">{etf.category}</span>
      </div>
      <h3 className="text-[13px] font-bold text-slate-100 mb-1.5 leading-snug">{etf.name}</h3>
      <p className="text-[11px] text-slate-400 leading-relaxed opacity-80 line-clamp-2">{etf.description}</p>
    </div>
  );
};
